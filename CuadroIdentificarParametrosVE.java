...
/**
 * Permite seleccionar el parámetro del método que contiene la estructura , ya sea divide 
 * y venceras o basada en arboles para un determinado algoritmo.
 */
public class CuadroIdentificarParametrosVE extends Thread implements
		ActionListener, KeyListener, MouseListener {

	private static final int ANCHO_CUADRO = 590;
	private static final int ALTO_CUADRO = 360;
	...
	public CuadroIdentificarParametrosVE(Ventana ventana,
			CuadroSeleccionMetodosVE cuadroSeleccionMetodosVE, MetodoAlgoritmo metodo, int numMetodo,
			String e, String ind) {
		this.metodo = metodo;
	...
	public void run() {
		...
		mensaje.setText(Texto.get("PRIMERAPREGUNTA_VISTASESPECIFICAS",
				Conf.idioma));
		panelBotones.add(mensaje);
		for(int i=0; i<2 ; i++) {
			this.botonesSeleccion[i] = new JCheckBox();
			if(i==0) {
				this.botonesSeleccion[i].setText(Texto.get("BOTON_MAXVISTASESPECIFICAS", Conf.idioma));
			}
			else if(i==1) {
				this.botonesSeleccion[i].setText(Texto.get("BOTON_MINVISTASESPECIFICAS", Conf.idioma));
		...
		mensaje2.setText(Texto.get("INDICACION_VISTASESPECIFICAS",
				Conf.idioma)+ (this.metodo.getNumeroParametros() - 1) + "):");
		panelBotones.add(mensaje2);
		GridLayout layout1 = new GridLayout(2, 1);
		
		
		GridLayout layout2 = new GridLayout(3, 1);
		JPanel panelIndicesGlobal = new JPanel(layout2);
		for(int i=0; i<3 ; i++) {
			JPanel panelIndices = new JPanel(new BorderLayout());
			this.camposVE[i] = new JTextField(this.LONGITUD_CAMPOS);
			this.camposVE[i].setBounds(10,10,200,30);
			
			
			JLabel mensajes = new JLabel();
			if (i == 0) {
				mensajes.setText(Texto.get("SOLPARCIAL_VISTASESPECIFICAS", Conf.idioma));
			} else if (i == 1) {
				mensajes.setText(Texto.get("MEJORSOL_VISTASESPECIFICAS", Conf.idioma));
			} else if (i == 2) {
				mensajes.setText(Texto.get("ESTIM_VISTASESPECIFICAS", Conf.idioma));
			}
		...
		//this.panelCuadro.add(panelIndicesGlobal, BorderLayout.CENTER);
		//this.panelCuadro.add(this.panelIzqda, BorderLayout.SOUTH);
		//this.panelCuadro.add(txt, BorderLayout.NORTH);
		...
		this.dialogo.setSize(CuadroIdentificarParametrosVE.ANCHO_CUADRO,
		...
		case 2:
			...
			for (int i = 0; i < this.numCampos; i++) {
				this.camposIndices[i] = new JTextField(
						CuadroIdentificarParametrosVE.LONGITUD_CAMPOS);
